...
**手动发布流程（workflow_dispatch）**：发布行为由 `publish-github` / `sync-halo` 显式开关决定 → 构建 + 质量门 → 上传/更新 Release（`publish-github=true` 时 tag 可留空自动推导；填写基础版本号时会自动补全 `v` 前缀并追加 `version-suffix`，也支持直接填写完整 tag）。
