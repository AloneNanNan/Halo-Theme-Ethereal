...
- **Halo FormKit 已知坑**：互斥 `if` 条件的同类型字段必须加唯一 `key`，否则 Vue 会复用组件实例导致设置值丢失（`settings.yaml` 里已有先例）。侧边栏小组件（`widgetsConfig.widgets` / `rightWidgets`）的条件字段已按 `widget_<部件>_<字段>` / `right_widget_<部件>_<字段>` 全部补齐 `key`——缺 `key` 时切换「部件」下拉会把旧部件的值串到新部件字段上（如音乐的「平台=网易云音乐」串进一言的「API 地址」）。新增小组件配置项时务必同步加唯一 `key`。
