...
- **搜索结果摘要显示 HTML 标签文本**：修复搜索摘要中正文的 HTML 标签（如 `<strong>`）被当作字面文本显示的问题，将其替换为空格，现仅保留搜索高亮标记 #64

- **照片详情页首尾照片提示**：修复图库详情页处于第一张/最后一张照片时，上一张/下一张按钮空白无提示的问题，与文章页提示体验保持一致 #62

- **三栏布局右侧栏自动收列回退**：回退 v1.2.1 引入的三栏布局下右侧栏无组件时自动切换为两栏显示的行为，三栏布局恢复始终保留第三列，布局只跟随「页面布局」设置 #33
