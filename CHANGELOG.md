...
- **三栏布局空右栏自动收列**：右侧栏无组件（个人简介/公告非右置、小组件列表清空为空）时，整页按两栏布局渲染——第三列轨道释放、页宽回落默认值；文章页目录同步改走两栏浮动目录（`#toc-container`），悬浮按钮与移动端弹窗目录逻辑不变
- **前台语言切换面板**：导航栏新增语言切换面板（简体中文 / 繁體中文 / English），写 Halo language cookie 后整页重渲染，当前语言高亮
- **语言设置项**：新增后台「语言切换」开关与「主题语言 → 默认语言」设置

### 优化

- **全站 i18n 补齐**：补齐页面副标题、随机访问/随机钓鱼按钮、申请友链模态框、文章操作栏、心愿颜色、侧边栏组件、四个模态框（状态/分享/打赏/外链）等缺失翻译；收敛全局 i18n 助手（`__etherealI18n` / `__etherealLangTag` / `__etherealSetLanguage` / `__etherealBigNum`），移除死词条与冗余 i18nResources 注入

### 修复

- 修复词条字面花括号 MessageFormat 转义坑
